...
public class Api {
    ...
    public void pay(String player, int amount, boolean message) {
        if (CurrencyMain.balance.contains(player)) CurrencyMain.balance.put(player, Integer.valueOf(((Integer) CurrencyMain.balance.get(player)).intValue() - amount));
        if (message) CurrencyMain.transfers.add(new Transfer(null, player, Math.abs(amount), Transfer.MONEY_GIVE));
        else
    ...
    public void pay(String sender, String receiver, int amount, boolean message) {
        if (CurrencyMain.balance.contains(sender)) CurrencyMain.balance.put(sender, Integer.valueOf(((Integer) CurrencyMain.balance.get(sender)).intValue() - amount));
        if (message) CurrencyMain.transfers.add(new Transfer(sender, receiver, Math.abs(amount), Transfer.PLAYER_PAYMENT));
        else
