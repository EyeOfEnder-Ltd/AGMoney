...
public class CurrencyListeners implements Listener
{
	...
	public CurrencyListeners(CurrencyMain currencyMain)
	{
	...
	public void onKill(PlayerKilledEvent event)
	{
		if (event.getKiller() == null)
			return;
		CurrencyMain.api.pay(event.getKiller().getName(), (int) Math.ceil(HungergamesApi.hgInstance.currentTime * 1000 / 1800), false);
	}
...
//		if ((user.getRank().intValue() > 1) && (event.getChannel().equals("#ServerChat")))
//		{
//			String[] msg = event.getMessage().split(":");
//			
//			if ((msg[0].equals(Bukkit.getMotd().replaceAll(" ", "").replaceAll("\\d*$", ""))) && (msg[1].equals("Voted")) && (msg.length == 5))
//			{
//				int amount = Integer.parseInt(msg[3]);
//				String name = msg[2];
//				if (Bukkit.getPlayerExact(msg[2]) != null)
//				{
//					if (Bukkit.getPlayerExact(msg[2]).getName().toLowerCase().equals(msg[2].toLowerCase()))
//					{
//						name = Bukkit.getPlayerExact(msg[2]).getName();
//						CurrencyMain.refreshers.add(name);
//						CurrencyMain.balance.put(name, Integer.valueOf(((Integer) CurrencyMain.balance.get(name)).intValue() + amount));
//					}
//				}
//				Bukkit.broadcastMessage(ChatColor.GREEN + msg[4].replace("$name", name).replace("$money", msg[3]));
//			}
//		}
//	}
}